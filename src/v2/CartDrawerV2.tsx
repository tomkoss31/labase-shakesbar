      ...
      >
        ...
        >
          ...
          {!empty && isAuthed && onClaimGift && (() => {
            // Source unique : REWARDS_CATALOG (aussi utilisé par la modale
            // récompenses). Les coûts/ids doivent rester alignés sur le backend
            // (api/orders.ts REWARDS) — le débit réel se fait côté serveur via
            // l'id, un coût divergent ferait échouer le claim en silence.
            const GIFTS = REWARDS_CATALOG.map((r) => ({
              id: r.id,
              title: r.title,
              emoji: r.emoji,
              cost: r.cost,
